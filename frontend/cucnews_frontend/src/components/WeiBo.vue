...
<div>
  <el-table
      ...
      <template slot-scope="props">
		  <el-form label-position="left" inline class="demo-table-expand">
		  			<el-form-item label="图片" style="width: 200px;">
		  			  ...
		  			  :src="props.row.original_pictures"
		  			  :preview-src-list="[props.row.original_pictures]"></el-image>
		  			</el-form-item>
		  </el-form>
        <el-form label-position="left" inline class="demo-table-expand">
	...
	<el-table-column
	  label="发布时间"
	  prop="publish_time" width="150">
	</el-table-column>
	<el-table-column
	  label="发布工具"
	  prop="publish_tool" width="150">
	</el-table-column>
	<el-table-column
	  label="点赞数量"
	  prop="up_num" width="150">
	</el-table-column>
	<el-table-column
	  label="转发数量"
	  prop="retweet_num" width="150">
	</el-table-column>
	<el-table-column
	  label="评论数量"
	  prop="comment_num" width="150">
	</el-table-column>
<!-- 	<el-table-column
	  label="contentUrl"
...
<script>
  ...
  export default {
	...
	methods:{
		getWeibo(){
			api.getWeibo({offset:(this.currentPage-1)*this.pageSize,limit:this.pageSize}).then(res => {
			  console.log(res)
			  if (res.status !== 1) {
			    this.loading = false
			    this.$message.error('获取失败')
			  }
			  this.tableData = res.weibo_list
